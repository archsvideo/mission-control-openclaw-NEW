...
export default function MarketingPage() {
  ...
  const funnelStages = [
    { label: "Impressions", value: active.reduce((s, c) => s + c.impressions, 0), icon: Eye },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Avg ROAS</CardTitle></CardHeader><CardContent><span className={`text-2xl font-mono font-bold ${avgRoas >= 2 ? "text-success" : avgRoas >= 1 ? "text-warning" : "text-destructive"}`}>{avgRoas.toFixed(1)}x</span></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Conversions</CardTitle></CardHeader><CardContent><span className="text-2xl font-mono font-bold">{totalConversions}</span></CardContent></Card>
      </div>
      <Card>
      ...
      <Card>
        ...
        <CardContent className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Creative</TableHead><TableHead>Platform</TableHead><TableHead>Status</TableHead>
                <TableHead className="text-right">Spend</TableHead><TableHead className="text-right">CTR</TableHead>
                <TableHead className="text-right">CPA</TableHead><TableHead className="text-right">ROAS</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {campaigns.map((c) => (
