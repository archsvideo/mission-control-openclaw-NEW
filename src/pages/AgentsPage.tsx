          ...
          <Card key={agent.id} className="relative overflow-hidden">
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div><span className="text-muted-foreground">Type:</span> <span className="font-medium">{agent.type}</span></div>
                ...
                <div><span className="text-muted-foreground">Last active:</span> <span className="font-mono text-xs">{new Date(agent.lastActive).toLocaleTimeString()}</span></div>
              </div>
              <div className="flex gap-2 pt-1">
                {agent.status === "running" ? (
                  <Button size="sm" variant="outline" onClick={() => toast.info(`Pausing ${agent.name}...`)}><Pause className="h-3 w-3 mr-1" /> Pause</Button>
                ) : (
                  <Button size="sm" variant="outline" onClick={() => toast.info(`Starting ${agent.name}...`)}><Play className="h-3 w-3 mr-1" /> Start</Button>
                )}
                <Button size="sm" variant="ghost" onClick={() => toast.info(`Restarting ${agent.name}...`)}><RotateCcw className="h-3 w-3 mr-1" /> Restart</Button>
              </div>
            </CardContent>
          </Card>
        ))}
