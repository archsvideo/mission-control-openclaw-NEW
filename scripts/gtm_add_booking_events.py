...
OUT = Path(r"C:\Users\Oscar\.openclaw\workspace\reports\gtm-booking-events-apply-2026-03-18.json")
...
result = {"created": {"triggers": [], "tags": []}, "existing": {"triggers": [], "tags": []}, "errors": [], "version": None}
...
def ensure_custom_event_trigger(event_name):
    ...
    for t in triggers:
        if t.get('name') == name:
            result['existing']['triggers'].append(name)
            return t
...
def ensure_ga4_tag(tag_name, event_name, trigger_id):
    ...
    for t in tags:
        if t.get('name') == tag_name:
            result['existing']['tags'].append(tag_name)
            return t
...
try:
    v = svc.accounts().containers().workspaces().create_version(
        parent=WS,
        body={"name": "Booking Events Fix", "notes": "Add booking custom events + calendly click tracking"}
    ).execute()
    result['version'] = {
