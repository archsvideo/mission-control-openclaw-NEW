...
open_rows = "\n".join([
    f"<tr><td>{p.get('timestamp','')}</td><td>{p.get('source_trader_model','')}</td><td>{p.get('pair','')}</td><td>{p.get('direction','')}</td><td>{p.get('entry',''):.6f}</td><td>{p.get('stop_loss',''):.6f}</td><td>{p.get('take_profit',''):.6f}</td><td>{p.get('reason_entry','')}</td></tr>"
...
<div class='card'><h3>Open simulated positions</h3><table><tr><th>Timestamp</th><th>Model</th><th>Pair</th><th>Dir</th><th>Entry</th><th>SL</th><th>TP</th><th>Reason</th></tr>{open_rows}</table></div>
